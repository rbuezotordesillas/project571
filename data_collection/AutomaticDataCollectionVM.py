...
if __name__ == "__main__":
    #import accounts
    accounts_df = pd.read_csv('~/project571/data_collection/DataVM1/accounts.csv',sep=";")
    #collect data
    twitter = get_twitter()
    ...
    #save Data
    todaysFollowers.to_csv('~/project571/data_collection/DataVM1/todaysFollowers_'+date+'.csv',sep=';',index=False)
    ##add to historic data
    historicData = pd.read_csv('~/project571/data_collection/DataVM1/todaysFollowers_all.csv',sep = ';') #load historic
    historicData = pd.concat([historicData,todaysFollowers[date]],axis=1) #add todays column
    historicData.to_csv('~/project571/data_collection/DataVM1/todaysFollowers_all.csv',sep=';',index=False) #save
